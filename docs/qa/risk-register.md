...
| R-09 | AI assistant abused (off-topic/injection), hallucination, or cost/latency | 2 | 2 | Layered fail-closed guardrail — L0 input caps, L1 flash-lite topic gate, L2 hardened system prompt + SafetySettings, L3 output validation (ADR-0017); guardrail evals (`internal/gemini`, `@trace FR-090..093`); per-user rate limit; keyless WIF (no secret); opt-in `AI_ENABLED` + 503 degradation. | mitigated |
| R-10 | Doc-graph drift — bootstrap docs point at renamed/missing files | 2 | 1 | `scripts/check-doc-links.mjs --check` (CI-gated + Stop hook); found+fixed `product-brief`→`project-brief`. | mitigated |
